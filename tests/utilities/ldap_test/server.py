...
JVM_SERVER_BIN = os.path.join(
    PACKAGE_DIR, "ldap-test-server-0.0.4-SNAPSHOT-jar-with-dependencies.jar"
)
...
def run_jvm_gateway(
    gateway_port=DEFAULT_GATEWAY_PORT, python_proxy_port=DEFAULT_PYTHON_PROXY_PORT
):
    try:
        log.info("Starting the JavaGateway on python_proxy_port %s", python_proxy_port)
        return JavaGateway(
            gateway_client=SlowGatewayClient(port=gateway_port),
...
def run_jvm_server(gateway_port=DEFAULT_GATEWAY_PORT):
    ...
    try:
        return subprocess.Popen(
            '"%s" -jar "%s" --port "%s"'
            % (jre_executable, JVM_SERVER_BIN, gateway_port),
            shell=True,
        )
    except OSError as e:
...
class MuffledGatewayConnection(GatewayConnection):
    def start(self):
        try:
            ...
            msg = "An error occurred while trying to connect to the Java " "server"
...
class ConfigBuilder(object):
    ...
    def _set_entries(self, config, entries):
        config.entries(
            self._list_to_array(
                self.gateway.jvm.com.github.trevershick.test.ldap.annotations.LdapEntry,
    ...
    def _map_to_entry(self, entry):
        ...
        for attr_name in attributes:
            ...
            attr_objects.append(
                self.builder.attribute(
                    attr_name, self._list_to_array(self.gateway.jvm.String, value)
                )
            )
        ...
        return self.builder.entry(
            ...
            self._list_to_array(
                self.gateway.jvm.com.github.trevershick.test.ldap.annotations.LdapAttribute,
                attr_objects,
            ),
        )
